...
void _kernel(const int M_thread, const int K, const int N_thread, const int N,
  ...
  for (int m = 0; m < M_thread / tile_row / group_size; m++) {
    for (int n = 0; n < N_thread / tile_col_fp32 / group_size; n++) {
      // load C
      _tile_loadd(
          0, C + m * group_size * tile_row * N + n * group_size * tile_col_fp32,
          N * sizeof(FP32));
      _tile_loadd(1,
...
void multithread(const int M, const int K, const int N, const BF16* A,
  ...
  for (int i = 0; i < num_threads; i++) {
    ...
    FP32* local_C = C + thread_x * M_per_thread * N + thread_y * N_per_thread;
    if (i == num_threads - 1) {
      _kernel(M_per_thread, K, N_per_thread, N, local_A, local_B, local_C);
      continue;
    }
