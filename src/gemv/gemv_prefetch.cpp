 ...
 */
constexpr int x_dist = 64;        // 64 / sizeof(BF16) * 3;
constexpr int y_dist = 64;        // 64 / sizeof(FP32) * 3;
constexpr int A_dist = 1024 * 5;  // tile_size * sizeof(BF16) * 1;
    ...
    for (i = 0; i <= M - 16 * 2; i += 16 * 2) {
        ...
        for (j = 0; j <= N - 32 * 2; j += 32 * 2) {
            ...
            for (int k = 0; k < 32 * 2; k += 64 / sizeof(BF16)) {
                _mm_prefetch((const char*)(x + j + 64 + x_dist + k),
                             _MM_HINT_ET0);
            }
            for (int k = 0; k < tile_size * 4; k += 64 / sizeof(BF16)) {
                _mm_prefetch((const char*)(A_pointer + A_dist + k),
                             _MM_HINT_T0);
            }
        }
