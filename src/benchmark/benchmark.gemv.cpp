...
void _init_benchmark(int M, int N, BF16* A, BF16* x, FP32* y) {
    random_buffer(A, M, N);
    random_buffer(x, N, 1);
    init_buffer(y, (FP32)0, M, 1);  // y0 is the reference
}

void _init_benchmark_multithreads(int num_threads, int M, int N, BF16* A,
                                  BF16* x, FP32* y) {
...
void benchmark_gemv(int M, int N, int max_iter) {
    ...
    random_buffer(x, N, 1);
    init_buffer(y1, (FP32)0, M, 1);
    duration = measure_time<std::chrono::microseconds>(max_iter, gemv_prefetch,
                                                       M, N, A, x, y1);
...
void benchmark_gemv_with_preprocess(int M, int N, int max_iter) {
    BF16* A = new BF16[M * N];
    BF16* x = new BF16[N];
    FP32* y0 = new FP32[M];
    FP32* y1 = new FP32[M];
    ...
    bind_core(num_threads);
    duration = measure_time_with_preprocess<std::chrono::microseconds>(
        max_iter, multithread_gemv, _init_benchmark_multithreads, num_threads,
        M, N, A, x, y1);
    print_result("Multithread version", max_iter, duration, ops_per_iter);
    // compare_buffer_max(y0, y1, M, 1, 10)
    //     ? std::cout << "Correctness: True" << std::endl
